...
Conflicting Statements Between the Two Documents
Here are the direct conflicts:

#	Topic	FUNCTIONAL_SPECIFICATION.md (SilentWake)	TFC-FUNCTIONAL-SPECIFICATION-v2.md (TFC)
1	Roles / Seats	3 player types: Admin, CO, 2 scenario-defined officers (NAV, Submarine)	7 ship seats (CO + 6 advisors: OPS, PWO, AAWO, CyOp, NAV, EO) + optional GM
2	Officer authority	Officers submit cards to a shared battleground; CO selects from submitted cards only	Advisors submit non-binding recommendations; CO selects from all available blue cards
3	CO card limit	Constrained by a per-turn score threshold (total score of committed cards must not exceed threshold)	Selects up to 2 blue cards per turn (hard cap, no score threshold)
4	Game phases	3 states: Waiting → Playing → Scoring	5 phases: Setup → Briefing → Running ↔ Paused → Completed
5	Turn timer	Fixed 120 seconds per turn	Stress-dependent: 5:00 at stress 0, decaying to 3:00 at stress 10
6	Score visibility	Score/breakdown shown in scoring phase; threshold shown during play	Score is never shown to players, even at end — only a tier label with positive messaging
7	Scoring tiers	<40% "Needs Improvement", 40–79% "Good Performance", ≥80% "Outstanding"	<33% "Solid Effort", 33–66% "Great Performance", ≥66% "Outstanding" (no negative wording)
8	Max score calc	Sum of highest-scoring card per role per turn	Scenario-defined ceiling (fixed value)
9	Domains	6 stress-counter domains: NAV, SENS, FIRE, COMMS, AAW, WPN	4 warfare threat-level domains: ASUW, ASW, AAW, CYBER
10	Domain tracking	Per-domain stress counters (0/1–2/3+ → Green/Orange/Red)	Per-domain threat levels (No threat / Possible / Actual) set by injects and cards
11	Ship systems	Not modeled — only domain stress counters exist	6 named systems + 5 weapons with power (ON/OFF) and operational state (Green/Yellow/Red)
12	Default scenario turns	10 turns	16 turns (Turn 0 briefing + Turns 1–15)
13	Stress visual	Segmented 10-bar with color zones (blue/orange/red)	Vignette darkening, screen shake, heartbeat pulse — environmental screen effects
14	Game modes	Single mode only	Classic mode (GM-driven, no scoring) and Collaborative mode (auto-sequenced, scored)
15	Player count	Minimum: all roles filled (admin + CO + all officers)	Full team (7), Two-player, or Practice (solo) modes
16	Facilitator during play	Admin observes from lobby, does not participate	GM has full control panel: triggers injects, pauses, adjusts speed, closes decisions
17	Communication	Integrated chat channel as core mechanic	No chat system mentioned; communication is verbal/external
18	Timeout behavior	Auto-resolves with whatever is committed; if nothing committed, no actions taken	Auto-submits the worst option (lowest score)
19	Inject lifecycle	Simple per-turn text shown in a modal at turn start	Full lifecycle: SCHEDULED → PENDING → RUNNING → COMPLETED, with dependencies and effects
20	Card count	~32 cards in global library	23 cards
21	Multi-domain cards	Officer chooses which single domain to target when playing a multi-domain card	Cards carry system effects (power/operational state changes); system targeting via picker
22	Deck Builder	Standalone companion tool for card design	Not mentioned
23	Defect system	Not present	Full defect lifecycle: INACTIVE → ACTIVE → MITIGATED → RESOLVED
24	Time model	Wall-clock countdown (120s per turn)	Dual-clock: Real Time + Play Time (RT × speed factor), adjustable mid-exercise
25	Scenario editor	Built-in form editor with live preview	Scenario Builder with tabs (Setup, Roles, Injects, Decisions, Initial States, Defects, Turns)
26	History/results	Game history archive with score, roster, scenario ID	Review View with timeline replay, play/pause/step, speed adjustment, decision analysis
27	Turn 0	Not present — turns start at 1	Explicit Turn 0: 15-minute pre-sail briefing, no blue cards
28	Forced cards	Not mentioned	Facilitator can force cards (e.g., SWB20 at Turn 6 auto-added if CO omits it)
Now let me create the merged document.

Write MERGED-FUNCTIONAL-SPECIFICATION.md
1057 lines
# SilentWake (TFC) — Merged Functional Specification
...
SilentWake is a **facilitator-controlled, turn-based tabletop cyber wargame** conducted digitally in real time. It is designed for training and evaluating team decision-making under pressure. It recreates the operational dynamics of a warship crew navigating hostile scenarios, where asymmetric information, time pressure, and cascading system stress force realistic collaboration between officers.

It is **not** a competitive multiplayer game — there is no physics simulation, no spatial model, and no inter-team scoring. The focus is on **decision rationale, communication under pressure, and trade-offs**.

The system is purpose-built for naval command staff training, cyber-defence evaluation, and operational readiness exercises.
...
**Teams:**
- **WHITE/RED TEAM (Facilitator / Game Master)** — runs the exercise, delivers injects, adjudicates outcomes.
...
Scenarios may define additional or alternative officer roles. Role names are normalized to lowercase with underscores.
...
- Manages the pre-mission lobby: selects, creates, edits, and loads scenarios.
- Authorizes mission start once the full team is assembled.
- Resets the game and returns all players to the lobby after the mission ends.
- The only role permitted to create, edit, and save scenario definitions.
...
- **ID** — Unique identifier (e.g., SWB01).
- **Action / Title** — Short descriptive name (e.g., "Continue Mission", "Cyber Containment Posture").
- **Effect** — Narrative description of what the action achieves tactically.
...
- **Image** — Optional visual card artwork displayed in-game.
...
### 5.5 Multi-Domain Card Handling

When a card targets a specific system, a system picker dropdown is presented. The player must choose which system the card applies to. This choice determines which system receives the card's effects.
...
|-------|-------------|----------------|
| **Setup** | Exercise created, waiting for crew. Players join, claim roles, facilitator loads a scenario. | No |
...
| **Completed** | Terminal. Exercise is over. Crew sees outcome overlay with after-action report. | No |
...
Injects are the primary mechanism for creating asymmetric information. Each officer sees only their own inject(s) for the current turn. Injects are prefixed with "Eyes only:" and provide:

- Station-specific tactical intelligence.
- Situational warnings or opportunities.
- Psychological pressure points (e.g., "the merchant captain is panicking").
- Hints about the best course of action for the turn.

Officers must communicate relevant inject information to the CO and each other through the chat channel or verbal communication.
...
- **Segmented stress bar** — 10-segment visual indicator with colour-coded zones:
  - Segments 0–3: blue (nominal).
  - Segments 4–6: orange (elevated).
  - Segments 7–9: red (critical).
...
### 13.4 After-Action Report

At mission end, the following information is available:
- Tier label and narrative assessment message.
- Per-turn score and stress breakdown (facilitator/review view only).
- Full team roster with role assignments.
- The game history record is persisted to the server.
...
- The name is stored in the browser session so reconnection is automatic. The previously claimed role is also stored and automatically re-claimed on reconnect.
...
- When a player disconnects, they are removed from the active player list.
- Their role becomes available for others to claim.
- The game state is broadcast to all remaining players.
...
A persistent text-based communication channel is available in all game phases (lobby and mission).

- All connected players share a single channel.
- Messages display the sender's name, role, and timestamp.
- Own messages are visually distinguished from others.
- An unread message counter appears for new messages during the mission phase.
- The unread count resets when the player scrolls the chat panel.
- The chat panel width is resizable by dragging its border.

### 17.2 Communication as Game Mechanic

Because each officer receives private injects, the chat channel is the primary in-game mechanism for building a shared tactical picture. Effective communication is a deliberate part of the evaluation — teams that fail to share critical station intelligence will make suboptimal decisions.

---

## 18. Real-Time Connection Model

### 18.1 Connection Types
...
### 18.3 Role-Targeted Broadcasting
...
When a mission ends (transitions to completed), a history record is automatically saved containing:
- Unique record ID and timestamp.
- Scenario ID and title.
- Total score achieved and maximum possible score.
- Full player roster with names and roles.

### 22.2 History Archive

All completed mission records are stored in a persistent server-side archive, preserving a chronological log of all simulation runs.

---

## 23. Scenario Validation Rules
...
9. Mission metadata contains a non-empty ID, title, difficulty, and description.
10. Every turn has a title, context, and numeric stress value.
11. Every turn has at least one inject entry for every targeted role.
12. Every turn has at least one card assignment (with a valid card ID and numeric score) for every role.
...
**Shipped scenarios:**
- **Silent Wake** (15 turns) — full cyber-kinetic exercise: transit → cyber detection → escalation → combat → recovery. Difficulty: Medium. Stress curve starts at 0 and escalates to +4 base stress per turn by the final phase.
- **Silent Wake Tutorial** (3 turns) — low-stakes port approach teaching the turn cycle. Difficulty: Easy. Covers basic mechanics in a low-stakes setting.

---

## 25. Deck Builder (Companion Tool)

A standalone companion application for designing and managing the visual card deck.

### 25.1 Capabilities

- Edit the card list (add, remove, modify action cards with IDs, names, domains, effects, stress values).
- Import and export card data as portable files.
- Load a custom card template image.
- Visually edit text and icon layout zones on the card template.
- Import and export layout configurations.
- Generate individual card preview images locally.
- Generate the full card set as individual image files.
- Supports per-card style overrides.
- Domain icons render as built-in vector graphics.

### 25.2 Operation

- Operates as a self-contained browser application with no server dependency beyond initial file serving.
- No persistent storage — all work is managed through manual import/export.

---

## 26. Design Invariants

These define what "correct" means. Every change must preserve them:
...
5. **Asymmetric Information.** No single player has the complete picture. Success depends on communication.
6. **Time Pressure.** Stress-adjusted timers prevent overthinking and force decisive action.
7. **Escalating Stress.** Scenarios are designed so that stress accumulates, degrading the team's operating conditions and raising stakes in later turns.
8. **Facilitator Adjudication.** The administrator controls the meta-game (scenario choice, start, reset) while officers and the CO drive the tactical decisions.
9. **Extensibility.** The scenario editor and variable role system allow new training scenarios to be authored without changing the underlying system.

---

## 27. Backlog (Identified, Not Yet Implemented)

### 27.1 Shaped (Ready to Build)
...
Created MERGED-FUNCTIONAL-SPECIFICATION.md.

Conflict Resolution Summary
Where the two docs conflicted, I used TFC v2 as the authoritative source (it's the more evolved/detailed spec). Key resolutions:

Conflict	Resolution
3 roles vs 7 seats	7 seats (CO + 6 advisors) from TFC
Officers submit cards vs recommend	Recommend (non-binding); CO picks from all available
Score threshold vs 2-card cap	2-card cap from TFC
3 phases vs 5 phases	5 phases (Setup/Briefing/Running/Paused/Completed)
Fixed 120s timer vs stress-dependent	Stress-dependent (5:00 → 3:00)
Score shown vs hidden	Hidden during play, tier label only at end
3 tiers (<40/40-79/80+) vs (<33/33-66/66+)	TFC tiers with positive-only messaging
6 domain stress counters vs 4 warfare domains	4 warfare domains (ASUW/ASW/AAW/CYBER) + ship systems
10 turns vs 16 turns	16 turns (Turn 0 + Turns 1–15)
No timeout auto-select vs worst-option	Worst option auto-submit on timeout
Content unique to each doc was preserved (chat channel from SilentWake, defect system/audit trail/game modes from TFC).
