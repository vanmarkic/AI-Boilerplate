...
test.describe("Score display — visible iff score exists and not briefing @player", () => {
  test("no turn banner when score is null", async ({ page }) => {
  ...
  test("no turn banner during briefing phase", async ({ page }) => {
    const SCORE_TURN_0 = {
      total_score: 0,
      stress: 0,
      turn_number: 0,
  ...
  test("turn banner visible when score exists and decision active", async ({
    page,
  }) => {
    await installMocks(
      page,
  ...
  test("turn banner shows turn number with active decision", async ({ page }) => {
    await installMocks(
      page,
      snapshot({
        score: SCORE_TURN_3,
...
test.describe("Advisor bubbles — DM sees recs, advisor does not @player", () => {
  test("decision-maker sees advisor bubbles when recs exist", async ({
    page,
  }) => {
  ...
  test("no bubbles when decision has zero recommendations", async ({
    page,
  }) => {
    await installMocks(page, snapshot({ decisions: [DECISION_OPEN] }));
    await page.goto(playerUrl("co-01", "co"));
