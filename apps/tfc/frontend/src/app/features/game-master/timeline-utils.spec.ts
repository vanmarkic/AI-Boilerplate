...
describe('timeline-utils', () => {
  describe('computeTimelineItems', () => {
    ...
    it('maps a scheduled event to a timeline item', () => {
      const events = [makeEvent({ id: 'e1', scheduled_pt_ms: 5000, duration_ms: 10_000 })];
    ...
    it('maps an active issue to a timeline item', () => {
      const issues = [makeIssue({
        ...
        activated_at_pt_ms: 3000, auto_resolve_ms: 20_000,
      })];
    ...
    it('uses playTimeMs for active issues with no auto_resolve', () => {
      const issues = [makeIssue({
        id: 'i1', lifecycle: 'active',
        activated_at_pt_ms: 2000, auto_resolve_ms: 0,
      })];
  ...
  describe('assignLanes', () => {
    it('assigns all non-overlapping items to lane 0', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 100, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'b', label: 'B', startMs: 100, endMs: 200, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'c', label: 'C', startMs: 200, endMs: 300, lifecycle: '', kind: 'event', lane: 0 },
      ];
    ...
    it('stacks overlapping items into separate lanes', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 200, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'b', label: 'B', startMs: 50, endMs: 150, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'c', label: 'C', startMs: 100, endMs: 250, lifecycle: '', kind: 'event', lane: 0 },
      ];
      const result = assignLanes(items);
    ...
    it('reuses lanes when items no longer overlap', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 100, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'b', label: 'B', startMs: 50, endMs: 150, lifecycle: '', kind: 'event', lane: 0 },
        { id: 'c', label: 'C', startMs: 150, endMs: 200, lifecycle: '', kind: 'event', lane: 0 },
      ];
      const result = assignLanes(items);
  ...
  describe('computeTimeScale', () => {
    ...
    it('uses 110% of max end time with items', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 100_000, lifecycle: '', kind: 'event', lane: 0 },
      ];
    ...
    it('uses playTimeMs if it exceeds all item ends', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 10_000, lifecycle: '', kind: 'event', lane: 0 },
      ];
    ...
    it('returns valid pxPerMs ratio', () => {
      const items: TimelineItem[] = [
        { id: 'a', label: 'A', startMs: 0, endMs: 60_000, lifecycle: '', kind: 'event', lane: 0 },
      ];
