        ...
        <div class="grid grid-cols-1 gap-sm">
          @for (scenario of scenarios(); track scenario.id) {
                ...
                <div class="flex flex-col gap-xs">
                  ...
                  <div class="flex gap-xs flex-wrap">
                    <ui-badge variant="secondary">
                      {{ scenario.content?.events?.length ?? 0 }}
                      {{ domain.term("event") }}s
                    </ui-badge>
                    <ui-badge variant="secondary">
                      {{ scenario.content?.issues?.length ?? 0 }}
                      {{ domain.term("issue") }}s
                    </ui-badge>
                    ...
                    <ui-badge
                      [variant]="
                        scenarioGameMode(scenario) === 'simple_collaborative'
                          ? 'default'
                          : 'outline'
                      "
                    >
            ...
            <p class="text-muted-foreground text-sm">
              ...
              <a href="/builder" class="text-primary underline"
                >Scenario Builder</a
              >.
            </p>
          }
