...
function makeContent(overrides: Partial<ScenarioContent> = {}): ScenarioContent {
  return {
    phases: [],
    events: [],
    issues: [],
...
describe('validateScenarioContent', () => {
  ...
  it('rejects roles without a decision_maker', () => {
    const errors = validateScenarioContent(
      makeContent({ roles: [{ id: 'nav', label: 'Nav', player_type: 'advisor' }] }),
    );
    expect(errors).toContain("At least one role must have player_type 'decision_maker'.");
  });

  it('accepts multiple roles with one decision_maker', () => {
    const errors = validateScenarioContent(makeContent({
      roles: [
  ...
  it('rejects decision template targeting unknown role', () => {
    const errors = validateScenarioContent(makeContent({
      decision_templates: [{
        id: 'd1', title: 'T', description: '', issue_id: 'i1',
        question_type: 'single_choice', options: [],
        completion_mode: 'first_response', target_roles: ['nonexistent'],
      }],
    }));
    expect(errors).toContain("Decision template 'd1' targets unknown role 'nonexistent'.");
  });
  ...
  it('accepts decision template targeting valid role', () => {
    const errors = validateScenarioContent(makeContent({
      decision_templates: [{
        id: 'd1', title: 'T', description: '', issue_id: 'i1',
        question_type: 'single_choice', options: [],
        completion_mode: 'first_response', target_roles: ['co'],
      }],
    }));
    expect(errors).toEqual([]);
  });
  ...
  it('returns multiple errors at once', () => {
    const errors = validateScenarioContent(makeContent({
      ...
      decision_templates: [{
        id: 'd1', title: 'T', description: '', issue_id: 'i1',
        question_type: 'single_choice', options: [],
        completion_mode: 'first_response', target_roles: ['ghost'],
      }],
    }));
