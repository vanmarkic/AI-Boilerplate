...
export class AnimationService {
  ...
  from(targets: gsap.TweenTarget, vars: gsap.TweenVars): gsap.core.Tween | null {
    if (this.reducedMotion()) return null;
    return gsap.from(targets, vars);
  }

  /** Stagger entrance animation for a list of elements. */
  staggerIn(targets: gsap.TweenTarget, vars?: gsap.TweenVars): gsap.core.Tween | null {
    if (this.reducedMotion()) return null;
  ...
  shake(target: gsap.TweenTarget, intensity = 2, duration = 0.3): gsap.core.Timeline | null {
    if (this.reducedMotion()) return null;
    const tl = gsap.timeline();
    tl.to(target, { x: intensity, duration: duration / 6, ease: 'power2.inOut' })
      .to(target, { x: -intensity, duration: duration / 6, ease: 'power2.inOut' })
      .to(target, { x: intensity * 0.6, duration: duration / 6, ease: 'power2.inOut' })
      .to(target, { x: -intensity * 0.6, duration: duration / 6, ease: 'power2.inOut' })
  ...
  pulse(target: gsap.TweenTarget, scale = 1.05): gsap.core.Tween | null {
    ...
    return gsap.fromTo(target,
      { scale: 1, opacity: 1 },
      { scale, opacity: 0.85, duration: 0.3, yoyo: true, repeat: 1, ease: 'power2.inOut' },
    );
  }
