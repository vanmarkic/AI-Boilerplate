...
def make_alert(
    ...
    observables: tuple[Observable, ...] = (IP,),
    labels: tuple[str, ...] = (),
    alert_id: UUID = ALERT_ID,
) -> Alert:
    """Build an alert to select a response for."""
    return Alert(
        alert_id=alert_id,
        event_id=EVENT_ID,
        dedup_key=dedup_key,
...
class TestSelect:
    ...
    def test_dropped_finding_runs_nothing(self) -> None:
        decision = select(make_alert(disposition=Disposition.DROP), CATALOG)
    ...
    def test_criticality_floor_is_enforced(self) -> None:
        ...
        standard = select(make_alert(), catalog)
        assert not standard.should_run

        crown = select(make_alert(AssetCriticality.CROWN_JEWEL), catalog)
    ...
    def test_selection_is_stable_across_calls(self) -> None:
        first = select(make_alert(), CATALOG)
        second = select(make_alert(), CATALOG)
        assert first == second
...
class TestIdempotencyKey:
    ...
    def test_selection_reuses_the_key_for_a_repeated_finding(self) -> None:
        ...
        first = select(make_alert(dedup_key="same"), CATALOG)
        second = select(make_alert(dedup_key="same"), CATALOG)
        assert first.idempotency_key == second.idempotency_key

    def test_distinct_events_get_distinct_keys(self) -> None:
        first = select(make_alert(dedup_key="a"), CATALOG)
        second = select(make_alert(dedup_key="b"), CATALOG)
        assert first.idempotency_key != second.idempotency_key
