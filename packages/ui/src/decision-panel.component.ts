  ...
  template: `
    <ui-dialog-panel (closed)="closed.emit()">
      ...
      <div class="decision-panel__options">
        @if (questionType() === 'single_choice') {
          @for (option of options(); track option.id) {
            <label class="decision-panel__option"
              [class.decision-panel__option--selected]="selectedOptions()[0] === option.id">
              <input type="radio" [name]="'decision-' + title()"
                [value]="option.id"
        ...
        @if (questionType() === 'multi_choice') {
          @for (option of options(); track option.id) {
            <label class="decision-panel__option"
              [class.decision-panel__option--selected]="selectedOptions().includes(option.id)">
              <input type="checkbox"
        ...
        @if (questionType() === 'free_text') {
          <textarea class="decision-panel__textarea"
            [value]="freeText()"
      ...
      <ng-container dialogFooter>
        <ui-button variant="outline" (clicked)="closed.emit()">Cancel</ui-button>
        <ui-button variant="default" (clicked)="onSubmit()"
          [disabled]="!canSubmit()">Submit</ui-button>
      </ng-container>
    </ui-dialog-panel>
  `,
})
export class DecisionPanelComponent {
  ...
  readonly submitted = output<{ selectedOptions: string[]; freeText: string }>();
