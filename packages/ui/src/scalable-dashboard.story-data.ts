import type { HistogramBar, HistogramLabel } from './histogram-timeline.component';
...
export function buildGroupsTemplate(
  groups: GroupDef[],
  defaultMode: 'aggregated' | 'disaggregated',
): string {
  return groups
    .map(
      (g) => `
      <ui-card-group
        ...
        [mode]="'${defaultMode}'"
      >
        <span groupSummary>${g.summary}</span>
        ${g.cards.map((c) => `<ui-card title="${c.title}">${c.body}</ui-card>`).join('\n        ')}
      </ui-card-group>`,
    )
    .join('\n');
}

export function buildFlatCardsTemplate(groups: GroupDef[], cols: number): string {
  const allCards = groups.flatMap((g) => g.cards);
  return `
    <ui-grid [cols]="${cols}" gap="md">
      ${allCards.map((c) => `<ui-card title="${c.title}">${c.body}</ui-card>`).join('\n      ')}
    </ui-grid>
  `;
}
