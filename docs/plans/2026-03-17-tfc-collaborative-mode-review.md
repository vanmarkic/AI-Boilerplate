...
| 3 | **Player identity hardcoded** | `engine_router.py` | **RESOLVED** | Waiting room passes `role` query param + `participantId`. Backend validates `participant_id` is non-empty. Wired through waiting room → player view → recommendation flow (PR #121). |
